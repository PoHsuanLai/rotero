...
pub async fn get_or_create_tag(conn: &Connection, name: &str, color: Option<&str>) -> Result<i64, turso::Error> {
    let mut rows = conn.query("SELECT id FROM tags WHERE name = ?1", [Value::Text(name.to_string())]).await?;
    ...
    conn.execute(
        ...
        turso::params::Params::Positional(vec![
            ...
            color.map(|c| Value::Text(c.to_string())).unwrap_or(Value::Null),
        ]),
    )
    .await?;
...
pub async fn list_tags(conn: &Connection) -> Result<Vec<Tag>, turso::Error> {
    let mut rows = conn.query("SELECT id, name, color FROM tags ORDER BY name", ()).await?;
...
pub async fn add_tag_to_paper(conn: &Connection, paper_id: i64, tag_id: i64) -> Result<(), turso::Error> {
    conn.execute(
