...
impl Database {
    ...
    async fn search_papers_fts(&self, query: &str) -> Result<Vec<Paper>, turso::Error> {
        ...
        let mut rows = self.conn.query(&sql, [Value::Text(query.to_string())]).await?;
    ...
    pub async fn list_papers(&self, offset: u32, limit: u32) -> Result<Vec<Paper>, turso::Error> {
        ...
        let mut rows = self
            .conn
            .query(&sql, [Value::Integer(limit as i64), Value::Integer(offset as i64)])
            .await?;
        let mut papers = Vec::new();
        while let Some(row) = rows.next().await? {
            papers.push(row_to_paper(&row));
    ...
    pub async fn set_favorite(&self, id: &str, favorite: bool) -> Result<(), turso::Error> {
        self.conn
            .execute(queries::PAPER_SET_FAVORITE, [Value::Integer(favorite as i64), Value::Text(id.to_string())])
            .await?;
        Ok(())
    }
    ...
    pub async fn set_read(&self, id: &str, read: bool) -> Result<(), turso::Error> {
        self.conn
            .execute(queries::PAPER_SET_READ, [Value::Integer(read as i64), Value::Text(id.to_string())])
            .await?;
        Ok(())
    }
    ...
    pub async fn list_annotations_for_paper(&self, paper_id: &str) -> Result<Vec<Annotation>, turso::Error> {
        let mut rows = self.conn.query(queries::ANNOTATION_LIST_FOR_PAPER, [Value::Text(paper_id.to_string())]).await?;
    ...
    pub async fn insert_note(&self, paper_id: &str, title: &str, body: &str) -> Result<String, turso::Error> {
    ...
    pub async fn list_paper_ids_in_collection(&self, collection_id: &str) -> Result<Vec<String>, turso::Error> {
        let mut rows = self.conn.query(queries::COLLECTION_PAPER_IDS, [Value::Text(collection_id.to_string())]).await?;
        let mut ids = Vec::new();
        while let Some(row) = rows.next().await? {
    ...
    pub async fn list_paper_ids_by_tag(&self, tag_id: &str) -> Result<Vec<String>, turso::Error> {
        let mut rows = self.conn.query(queries::TAG_PAPER_IDS, [Value::Text(tag_id.to_string())]).await?;
        let mut ids = Vec::new();
        while let Some(row) = rows.next().await? {
            if let Some(id) = get_opt_text(&row, 0) {
                ids.push(id);
    ...
    pub async fn get_or_create_tag(&self, name: &str, color: Option<&str>) -> Result<String, turso::Error> {
        let mut rows = self
            .conn
    ...
    pub async fn add_tag_to_paper(&self, paper_id: &str, tag_id: &str) -> Result<(), turso::Error> {
        self.conn
            .execute(queries::TAG_ADD_TO_PAPER, [Value::Text(paper_id.to_string()), Value::Text(tag_id.to_string())])
            .await?;
        Ok(())
    }
}
