...
///
/// This is what the app does, so tests share its behaviour: the schema
/// migrations all run, including the one that stamps existing rows.
...
/// A library with the app tables but no device identity — the shape a build that
/// skipped initialization left behind. Its rows cannot be attributed, so none of
/// them can win a merge.
